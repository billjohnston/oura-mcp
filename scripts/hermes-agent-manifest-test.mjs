...
const pinnedPackage = `oura-mcp-unofficial@${packageJson.version}`;
const dir = mkdtempSync(join(tmpdir(), 'oura-mcp-hermes-agent-'));

const client = new Client({ name: 'oura-mcp-hermes-agent-test', version: '0.0.0' });
...
try {
  ...
  assert.ok(toolNames.includes('oura_agent_manifest'), 'Hermes-ready agent manifest tool should be registered.');

  const resources = await client.listResources();
  const resourceUris = resources.resources.map((resource) => resource.uri);
  assert.ok(resourceUris.includes('oura://agent-manifest'), 'Agent manifest resource should be registered.');

  const manifestResult = await client.callTool({
    name: 'oura_agent_manifest',
  ...
  assert.equal(manifest.hermes.tool_name_prefix, 'mcp_oura_');
  ...
  assert.match(text, /mcp_oura_oura_connection_status/);
  ...
  assert.ok(setupPayload.hermes_skill_path.endsWith('.hermes/skills/oura-mcp/SKILL.md'));
  ...
  assert.equal(doctorPayload.client_checks.hermes.oura_server_configured, true);
  ...
  const mergeDir = mkdtempSync(join(tmpdir(), 'oura-mcp-hermes-merge-'));
  ...
  assert.match(mergedConfig, /oura:/);
