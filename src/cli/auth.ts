...
export async function runAuthCommand(args: string[]): Promise<number> {
  ...
  const result = await waitForOAuthCode(redirect, state, timeoutMs, async (url) => {
    if (!json) {
      console.log("Oura MCP · Authorization");
      console.log("");
      if (noOpen) {
        console.log("Open this URL manually:");
        console.log(`  ${url}`);
      } else {
        console.log("Opening Oura authorization in your browser...");
      }
      console.log("");
      console.log("Steps");
      console.log("  1. Approve access in the browser tab that opens.");
      console.log("  2. Oura will redirect to the local callback.");
  ...
  const output = {
    ...
    next_step: "Run `oura-mcp-server doctor`, then add the MCP server to your agent."
  };
  if (json) console.log(JSON.stringify(output, null, 2));
  else {
    console.log("");
    console.log("✓ Oura connected");
    console.log("");
...
function waitForOAuthCode(
  ...
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(() => {
      server.close();
      reject(new Error("Timed out waiting for OURA OAuth callback."));
    }, timeoutMs);
    ...
    const server = createServer((req, res) => {
      try {
        ...
        if (error) throw new Error(`Oura authorization failed: ${error}`);
        if (!code) throw new Error("OURA callback did not include a code.");
        if (state !== expectedState) throw new Error("OURA callback state mismatch.");
...
<head>
  ...
  <title>Oura connected · Delx Wellness</title>
  <style>
...
<body>
  ...
  <ol>
    ...
    <li>Run <code>oura-mcp-server doctor</code> to verify the setup.</li>
