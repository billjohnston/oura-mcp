...
import { MAX_OURA_LIMIT } from "../constants.js";
...
/** Sort key for "most recent". Oura returns collections oldest-first, so the newest is the tail. */
function recencyKey(record: unknown): string {
...
/** The newest record, or undefined when the window is empty. Falls back to the last element. */
function mostRecent(records: unknown[]): unknown {
...
async function latestReadinessResource(uri: URL) {
  ...
  // Read the window, then return only its newest record: this resource is singular by contract.
  // limit:1 alone would return the OLDEST record, because Oura serves collections oldest-first.
  const result = await new OuraClient(config).list(endpoint, { limit: MAX_OURA_LIMIT });
  const latest = mostRecent(result.records);
  const data = applyPrivacy(endpoint, { records: latest === undefined ? [] : [latest] }, resolvePrivacyMode(config));
...
export function registerOuraResources(server: McpServer): void {
  ...
  server.registerResource("oura_latest_readiness", "oura://latest/readiness", { title: "Latest Oura Readiness", description: "Most recent Oura readiness record in the configured privacy mode.", mimeType: "application/json" }, latestReadinessResource);
